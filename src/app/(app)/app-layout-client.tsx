...
export function AppLayoutClient({ children, mmsOnboarding }: AppLayoutClientProps) {
  const { user, profile, loading, profileFetchFailed, retryProfile } = useAuth()
  const router = useRouter()

  useEffect(() => {
    if (!loading && !user) {
      // Phone-OTP users have no email/password credentials, so sending them to
      // /auth would strand them. When the phone onboarding path is live, route
      // there instead; otherwise fall back to the classic email/password sign-in.
      router.push(mmsOnboarding ? '/onboarding/phone' : '/auth')
    }
  }, [user, loading, router, mmsOnboarding])
